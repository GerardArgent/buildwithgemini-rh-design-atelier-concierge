...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            data_val = root.data
            meta = getattr(root, "metadata", None) or (data_val.get("metadata") if isinstance(data_val, dict) else {}) or {}
            mime = meta.get("mimeType") if isinstance(meta, dict) else None
            if mime == _A2UI_MIME:
                # If wrapped in kind/data/metadata, unwrap inner data payload for the A2UI renderer
                inner_data = data_val.get("data") if (isinstance(data_val, dict) and "data" in data_val) else data_val
